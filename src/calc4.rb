...
class Interpreter
  class EndOfTextReached < StandardError; end
  ...
  def run
    expression
    raise EndOfTextReached
  rescue ValidationError => e
    p e
    exit
  rescue EndOfTextReached
    result
  end
  ...
  def determine_if_eof_reached
    raise EndOfTextReached if text[position].nil?
  end
  ...
  def operator_expected
    ...
    when '*'
      @operator = Token.new(MULTIPLY, current_character)
    when '/'
      @operator = Token.new(DIVIDE, current_character)
    end
